...
Factorized Attention 的想法是有 $p$ 个分离的注意力头，第 $m$ 个头定义了一个下标子集 $A^{(m)}_{i} \subset { j | j <= i }$ 并且使 $S_{i}=A^{(m)}_{i}$。我们期望能够高效地选择子集 $A$，使得 $|A^{(m)}_{i}|\propto \sqrt[p]{n} $ 通过这种方式来大幅降低计算复杂度。
